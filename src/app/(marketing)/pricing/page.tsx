...
const UNIFIED_MATRIX_CONFIG = [
  ...
  { label: "Custom Domain Options", matchKeywords: ["custom domain", "subdomain"] },
  ...
  { label: "Real-time Telematics", matchKeywords: ["telematics", "tracking"] },
    ...
    <div className="w-full m-auto min-h-screen">
      ...
      <section className="mt-8 bg-muted/80 px-4 pb-20 pt-8 sm:mt-10 sm:px-6 sm:pt-10 lg:mt-12 lg:px-8 lg:pt-12">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-start gap-5 md:grid-cols-3">
          {plans?.map((plan, index) => (
            <div key={index}
              className={`relative h-full flex flex-col rounded-2xl p-8 border border-brand-300 dark:border-brand-900 dark:text-white shadow-sm hover:-translate-y-5 ${selectedIndex === index ? '-translate-y-5 bg-zinc-900' : 'bg-transparent'} transition-all`}
              style={{ opacity: selectedIndex === index ? 1 : 0.85 }}
            >
              <span className={`mb-4 inline-block text-[10px] font-bold uppercase tracking-widest text-muted-foreground text-brand-400 ${selectedIndex === index ? 'text-green-500 border w-fit border-green-500 p-2 px-3 rounded-full' : 'text-black'}`}>{plan?.popular ? '● Most Popular' : plan?.name}</span>
              ...
              <a
                href="#"
                className="mb-7 flex w-full items-center justify-center gap-2 rounded-lg py-3 text-sm font-semibold bg-brand-500 text-white shadow-sm"
              >
                {plan.ctaText}
                  ...
                  {plan?.features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-2.5">
                      {feature?.included ? (
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-minus mt-0.5 shrink-0 text-red-500/40" aria-hidden="true">
                          <path d="M5 12h14"></path>
                        </svg>
                      )}
                      <span className={`text-[13px] ${selectedIndex === index ? 'text-gray-200' : 'text-muted'}`}>
                        <span className="font-medium text-foreground"><span className="font-semibold">{feature.highlightedText}</span> {feature.text}</span>
                      </span>
                    </div>
                  ))}
                </div>
      ...
      <section>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <h3 className="text-amber-500 text-center">Full Feature Comparison</h3>
          ...
          <div id="feature-comparison" className="mx-auto max-w-7xl bg-gray-100 dark:bg-zinc-900 p-3 px-4 rounded-2xl">
            <Table>
              ...
              <TableHeader>
                <TableRow className="border-b border-b-gray-400">
                  ...
                  {plans?.map((p, i) => (
                    <TableCell key={i} className="text-black p-2 dark:text-white text-center font-bold">{p.name}</TableCell>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
                ...
                <TableRow className="hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                  ...
                  {plans?.map((p, i) => (
                    <TableCell key={i} className="p-3 text-center text-base font-extrabold text-zinc-900 dark:text-white">
                ...
                {UNIFIED_MATRIX_CONFIG.map((row, index) => (
                  <TableRow key={index} className="hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                    ...
                    {plans.map((plan, planIndex) => {
                      ...
                      if (!targetFeature) {
                        return (
                          <TableCell key={planIndex} className="p-3 text-center">
                      ...
                      if (targetFeature.highlightedText) {
                        ...
                        // Static functional checks present get a unified clean icon check mark
                        displayValue = (
                      ...
                      return (
                        <TableCell key={planIndex} className="p-3 text-center text-sm text-zinc-700 dark:text-zinc-300">
                          {displayValue}
                        </TableCell>
                      );
                    })}
