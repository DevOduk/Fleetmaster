...
import { vehiclesBodyTypes, vehiclesCategories } from "@/data/globalExports";

interface VehiclePageProps {
  params: Promise<{ vehicleID: string }>;
}

const defaultVehicleD = {
  driver_type: "Self Drive",
...
const NewVehiclePage = () => {
  ...
  useEffect(() => {
    const loadVehicleProgress = () => {
      ...
      try {
        ...
        if (
          JSON.stringify(parsedProgress) !== JSON.stringify(VehicleDetails)
        ) {
        ...
        console.error(
          "Failed to parse vehicle progress from localStorage",
          error,
        );
      }
    };

    const timeoutId = setTimeout(loadVehicleProgress, 0);
  ...
  const handleCreateVehicle = async () => {
    ...
    const res = await createVehicleForTenant({
      ...VehicleDetails,
      tenant_id: profile?.tenant_id,
      owner: profile?.fleetmaster_tenants?.name,
    });
  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <div className="">
        ...
        <div className="mx-auto max-w-5xl space-y-6 lg:col-span-7">
          {sessionAlert && (
            <Alert
              variant="info"
              title="Continue where you left!"
              message="We noticed you did not complete you previous listing. You can take it from here ..."
            />
          )}
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div className="mb-6 flex items-start justify-between">
              <div>
                ...
                <p className="text-gray-500">
                  Body Type:{" "}
                  {VehicleDetails?.category || "Enter body type"}{" "}
                </p>
              </div>
              <div>
                <span
            ...
            <div className="relative">
              ...
              >
                <Chip
                  ...
                  label={VehicleDetails?.fuel_type || "Petrol"}
                />
                <Chip
                  sx={{ px: 1 }}
                  variant="filled"
              ...
              <img
                src={VehicleDetails?.image_url || "/images/default-yard.png"}
                alt={``}
            ...
            <div className="mt-6 grid grid-cols-2 gap-y-4">
              ...
              <div className="p-2">
                ...
                <div className="font-sm mt-2 mb-1 flex flex-wrap gap-3 dark:text-white">
                  {[
                    "Petrol/Gasoline",
                    "Diesel",
                    "Hybrid",
                    ...
                    <span
                      key={t}
                      className={`cursor-pointer rounded-lg px-4 py-2 text-sm ${t === VehicleDetails?.fuel_type ? "bg-brand-500 text-white" : "bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-300"}`}
                      onClick={() =>
                        setVehicleDetails((prev: any) => ({
                          ...prev,
                          fuel_type: t,
              ...
              <div className="p-2">
                <p className="text-gray-400">
                  Location (Cant't find location? Create one{" "}
                  <Link
                    target="_blank"
                    href={"/yards"}
            ...
            <div
              className={
                "mt-3 " +
                (!VehicleDetails?.tracker?.provider?.trim()
                  ? "hidden"
                  : "block")
              }
            >
