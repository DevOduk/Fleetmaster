...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (tenantName) {
    title = `Service & Maintenance | ${tenantName}: FleetMaster - Premium Car Rental & Fleet Solutions Software`;
    tenantDescription =
      tenantDescription ||
      `${tenantName} offers top-tier vehicle rentals. Book reliable vehicles across multiple locations easily.`;
  } else {
    title = `Service & Maintenance | FleetMaster - Premium Car Rental & Fleet Solutions Software`;
  }
...
export default function page() {
  return (
    <div>
      <PageBreadcrumb pageTitle="Service & Maintenance" />
      <ServiceCalendar />
    </div>
  );
}
