...
import SystemAdmins from "@/components/bookings/SystemAdmins";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (tenantName) {
    title = `System Admins | ${tenantName}: FleetMaster - Premium Car Rental & Fleet Solutions Software`;
    tenantDescription =
      tenantDescription ||
      `${tenantName} offers top-tier vehicle rentals. Book reliable vehicles across multiple locations easily.`;
  } else {
    title = `System Admins | FleetMaster - Premium Car Rental & Fleet Solutions Software`;
  }
...
export default async function Page() {
  return (
    <div>
      <PageBreadcrumb pageTitle="System Admins" />
      <SystemAdmins />
    </div>
  );
}
