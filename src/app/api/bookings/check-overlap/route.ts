...
export async function POST(request: Request) {
    try {
        ...
        if (!vehicleID || !rentalStart || !rentalEnd || !rentalTime || !tenantID || !userID) {
            return NextResponse.json(
                { error: "Missing required parameters for backend scheduling validation, including rentalTime and userID.", success: false },
                { status: 400 }
            );
        }

        // Clean and handle time string safely whether it comes as "11:35" or "11:35:00"
        let cleanTime = typeof rentalTime === 'string' ? rentalTime.trim() : String(rentalTime);
        ...
        if (isNaN(startDateObj.getTime()) || isNaN(endDateObj.getTime())) {
            return NextResponse.json(
                { error: `Invalid date or time format provided. Received start: ${rentalStart}, end: ${rentalEnd}, time: ${rentalTime}`, success: false },
                { status: 400 }
            );
        }
        ...
        const { data: potentialConflicts, error: overlapError } = await supabase
            ...
            .select('id, user_id, booking_status, payment_status, created_at, rental_start, rental_end, rental_time')
            .eq('vehicle_id', Number(vehicleID))
            .neq('booking_status', 'Cancelled');

        if (overlapError) {
            console.error("❌ Database overlap check crashed:", overlapError);
            return NextResponse.json({ error: "Failed to perform fleet calendar checks.", success: false }, { status: 500 });
        }

        let conflictingBookingInfo: { rental_start: string; rental_end: string; rental_time: string } | null = null;

        // Step B: Filter conflicts locally in Node memory including extension exception & buffer rules
        const activeOverlaps = potentialConflicts?.filter(booking => {
            if (!booking.rental_start || !booking.rental_end) return false;

            const bookingTime = booking.rental_time || rentalTime;
            let cleanBookingTime = typeof bookingTime === 'string' ? bookingTime.trim() : String(bookingTime);
            if (cleanBookingTime.split(':').length === 2) {
                cleanBookingTime = `${cleanBookingTime}:00`;
            }

            const existingStartObj = new Date(`${booking.rental_start}T${cleanBookingTime}`);
            const existingEndObj = new Date(`${booking.rental_end}T${cleanBookingTime}`);

            if (isNaN(existingStartObj.getTime()) || isNaN(existingEndObj.getTime())) {
                return false;
            }
            ...
            const hasTimeOverlap = requestedStartMS < bufferedExistingEndMS && requestedEndMS > bufferedExistingStartMS;
            ...
            if (booking.booking_status === 'Reserved') {
                if (booking.payment_status === 'PAID' || booking.payment_status === 'COMPLETE') {
                    isReservedBlocked = true;
                } else {
                    const bookingCreationTime = new Date(booking.created_at).toISOString();
        ...
        console.error("❌ Uncaught Critical Server Pipeline Exception:", globalError);
        return NextResponse.json(
            {
