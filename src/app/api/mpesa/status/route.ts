...
export async function POST(request: Request) {
    ...
    try {
        ...
        let response = await fetch("https://sandbox.safaricom.co.ke/mpesa/stkpushquery/v1/query", {
            method: "POST",
            headers: {
        ...
        if (data.fault && data.fault.detail?.errorcode === "policies.ratelimit.SpikeArrestViolation") {
            ...
            const retryTimestamp = new Date().toISOString().replace(/[^0-9]/g, "").slice(0, 14);
            const retryPassword = Buffer.from(shortCode + passkey + retryTimestamp).toString("base64");
            ...
            response = await fetch("https://sandbox.safaricom.co.ke/mpesa/stkpushquery/v1/query", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${accessToken}`,
