    ...
    try {
        ...
        if (!vehicleID || !rentalStart || !rentalEnd || !tenantID) {
            return NextResponse.json(
                { error: "Missing required parameters for backend scheduling validation." },
                { status: 400 }
            );
        }
        ...
        if (overlapError) {
            ...
            return NextResponse.json({ error: "Failed to perform fleet calendar checks." }, { status: 500 });
        }
        ...
        if (activeOverlaps.length > 0) {
            return NextResponse.json(
                { error: "This vehicle is occupied or reserved during your selected dates. Please try again after 15 minutes or choose a different date!" },
                { status: 409 }
            );
        }
        ...
        return NextResponse.json({ error: "Critical server pipeline runtime crash.", message: globalError.message }, { status: 500 });
    }
}