...
async function sendOTPSMS(phone: string, otp: string) {
  ...
  try {
    ...
    if (!recipient || recipient.statusCode !== 101) {
      // 101 usually means Success/Processed
      throw new Error(
        recipient?.status || "Failed to dispatch SMS via Africa's Talking.",
      );
    }
...
export async function sendPhoneVerification(
  ...
  try {
    ...
    if (error) {
      ...
      return {
        success: false,
        error: { message: "Failed to set phone verification code.", error },
      };
    }
    ...
    try {
      ...
      return {
        ...
        success: false,
        error: {
          message: `Verification SMS failed to send: ${smsErr.message}`,
        },
      };
    }
...
export async function resendPhoneOTP(
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        error: { message: smsErr.message || "Failed to resend SMS code." },
      };
    }
  ...
  try {
    ...
    if (cachedOtpData) {
      ...
      const { data: dbUser, error } = await supabase
        .from(tableSource)
        .select(
          `id, first_name, phone, phone_otp_code, phone_otp_expires_at, verification_status, fleetmaster_tenants(slug, name)`,
        )
        .eq("id", id)
        .single();
      ...
      if (error || !dbUser) {
        return {
          ...
          error: {
            error,
            message: "Invalid phone number or account does not exist.",
          },
        };
      }
    ...
    if (!user) {
      const { data: dbUser, error } = await supabase
        .from(tableSource)
        .select(
          "id, first_name, phone, verification_status, fleetmaster_tenants(slug, name)",
        )
        .eq("id", id)
        .single();
    ...
    if (updateError) {
      ...
      return {
        ...
        error: {
          message:
            updateError.message || "Failed to complete phone verification.",
        },
      };
    }
    ...
    return {
      ...
      error: {
        err,
        message: err.message || "An error occurred during phone verification.",
      },
    };
  }
}
