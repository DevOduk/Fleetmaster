...
const AppHeader: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadSearchData = async () => {
      const [ticketsRes] = await Promise.all([fetchUserTickets(profile.id)]);
  ...
  const otherLinks = useMemo(
    () => [
      ...navItems.map((nav) => {
        return {
          title: nav.name,
          description: `${nav.name} | Open ${nav.name} Admin Page`,
          link: nav.path || "#",
        };
      }),
      ...othersItems.map((nav) => {
          ...
          description:
            nav.name === "Account Settings"
              ...
              : nav.name === "User Profile"
                ...
                : nav.name === "Support"
                  ? "Support Tickets | View, manage and track support tickets"
        ...
        <div className="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
          <div className="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4 dark:border-gray-800">
            <div className="flex items-center gap-2 ps-2">
              <button
                className="z-99 h-10 w-10 items-center justify-center rounded-lg border-gray-200 text-gray-500 lg:flex lg:h-11 lg:w-11 lg:border dark:border-gray-800 dark:text-gray-400"
            ...
            <button
              ...
              className="z-99 flex h-10 w-10 items-center justify-center rounded-lg text-gray-700 hover:bg-gray-100 lg:hidden dark:text-gray-400 dark:hover:bg-gray-800"
            >
              <svg
                width="24"
                height="24"
            ...
            <div className="2xsm:gap-3 flex items-center gap-2">
              {profile?.fleetmaster_tenants?.name && (
                <>
                  ...
                  <div className="h-10 w-0.5 rounded bg-gray-400 dark:bg-gray-600"></div>
                </>
              )}
              {/* <!-- Dark Mode Toggler --> */}
              <ThemeToggleButton />
