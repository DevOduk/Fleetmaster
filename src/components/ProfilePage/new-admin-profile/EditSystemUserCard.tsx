    ...
    <div className="col-span-2 lg:col-span-1 space-y-1 flex flex-col">
      <Label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">{label}</Label>
      {
        type === "select" ?
          ...
          :
          <Input placeholder={placeholder} error={label === "New Password"} hint={label === "New Password" && "Password must be atleast 8 characters with a combination of special character, numbers, and letters"} disabled={disabled} type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} className="h-9" />
      }
    </div >
  );
}