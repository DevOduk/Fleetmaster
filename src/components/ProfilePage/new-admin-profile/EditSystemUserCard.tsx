          ...
          <form className="flex flex-col">
            <div className="px-2 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <EditableInput placeholder="Select user timezone" type="select" label="Timezone" value={profileDetails?.timezone} onChange={(v) => handleInputChange("timezone", v)} />
                <EditableInput placeholder="Select user primary language" type="select" label="Language" value={profileDetails?.language} onChange={(v) => handleInputChange("language", v)} />
              </div>
            </div>
          </form>
...
function EditableInput({ label, value, onChange, type = "text", disabled, placeholder, options }: { label: string; value: string; onChange: (v: string) => void; type?: string; disabled?: boolean; placeholder?: string; options?: any[]; }) {
  return (
    <div className="col-span-2 lg:col-span-1 space-y-1 flex flex-col">
      ...
      {
        type === "select" ? <>
          <Select value={value || ""} placeholder={placeholder} onChange={(e) => onChange(e)} options={(options || []).map(m => { return { label: m, value: m } })}></Select>
        </> :
