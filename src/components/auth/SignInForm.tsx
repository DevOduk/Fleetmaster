...
export default function SignInForm({ tenant }: Tenant) {
  ...
  const handleSubmit = async () => {
    ...
    const isTenantManager = () => {
      ...
      if (host.includes("dashboard.") || pathname.includes("/tenant-manager"))
        return true;
      return false;
    };
    ...
    if (result.success) {
      ...
      // check if is otp and take to change password
      if (result.is_otp) {
        setIsRedirecting(true);
        // make link encoded and safe
        router.push("/profile/account-settings/change_password");
        return;
      }
          ...
          <div className="mb-5 sm:mb-8">
            {(profile || adminProfile) && (
              ...
              >
                ...
                {profile?.first_name || adminProfile?.first_name}
              </Link>
            )}
