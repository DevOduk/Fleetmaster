...
const revenueExpenses = ({ bookings, expenses }: any) => {
  ...
  const getESumForMonth = (data: any[], targetMonthIndex: number) => {
    return data
      .filter((item) => {
        ...
        // Match only items in the current year and the specific month
        return (
          getYear(date) === currentYear && getMonth(date) === targetMonthIndex
        );
      })
  ...
  const revenueData = categories.map((_, index) =>
    getSumForMonth(bookings, index),
  );
  const expensesData = categories.map((_, index) =>
    getESumForMonth(expenses, index),
  );
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 sm:px-6 sm:pt-6 dark:border-gray-800 dark:bg-white/3">
      {loading ? (
        <div className="h-125 space-y-6 rounded-xl border border-gray-900/60 bg-transparent p-5 xl:col-span-2">
          ...
          {/* Fake Bar Chart Bars */}
          <div className="flex h-100 items-end justify-between gap-2 px-2 pt-4">
            {[
              ...
              <div
                key={i}
                style={{ height: `${height}%` }}
                className="w-full animate-pulse rounded-t bg-gray-300 dark:bg-gray-600"
              />
            ))}
          </div>
        </div>
