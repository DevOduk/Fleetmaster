...
export default function DemographicCard({ clients }: { clients: any[]; }) {
    ...
    const countries = [...new Set(allClients?.map(client => client?.country).filter(Boolean))];
            ...
            <div className="space-y-5">
                {
                    loading ? <>
                        ...
                        <div className="flex flex-col gap-4">
                            {countries.map((c, i) => {
                                const percentage = ((allClients?.filter(m => m.country === c).length) / allClients.length) * 100;
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <span className="block w-fit text-nowrap text-gray-500 text-theme-xs dark:text-gray-400">
                                                    {(allClients?.filter(m => m.country === c).length.toLocaleString() || 0)} Clients
                                                </span>
                                            </div>
                                        </div>
