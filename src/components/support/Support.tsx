...
const Support: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (hasOpenTicket) {
      showToast('You already have an open support ticket. Wait for a response to creeate another!', 'error');
      setMode('Tickets');
      return;
    }
    ...
    const result = await submitSupportRequest(
      { ticket_number: ticketNumber, subject: issueTitle, description: messageTwo, category: category },
      { id: profile!.id, role: profile!.role || "Client", tenant_id: profile!.tenant_id }
    );
        ...
        <ComponentCard title="Contact Support Request">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="flex mt-3 ms-auto items-center justify-center p-2 px-3 font-medium text-white rounded-lg bg-brand-500 text-theme-sm hover:bg-brand-600 disabled:opacity-50"
            >
      ...
      {/* 2. Recent Tickets Section */}
      {
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-slate-800 dark:text-white">Recent Tickets</h2>
          ...
          <div className="overflow-x-auto mb-4">
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/5">
                {
                  !loading ? tickets.slice(startIndex - 1, endIndex).length > 0 ? tickets.slice(startIndex - 1, endIndex).map((t) => (
                    <TableRow key={t.id} className="text-sm">
                      <TableCell className="p-3 text-brand-500">{t.ticket_number}</TableCell>
                      <TableCell className="p-3 text-brand-500"><Badge color="info" >{t.category}</Badge></TableCell>
                      <TableCell className="p-3 text-slate-700 dark:text-slate-300">{t.subject}</TableCell>
