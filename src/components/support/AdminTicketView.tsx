...
import { getTicketDetails, addSupportResponse, updateTicket } from "@/app/actions/support";
...
export default function AdminTicketView() {
    ...
    if (!adminProfile || loading || fetching) {
        return (
            <div className="space-y-6">
                <div className="w-full h-10 bg-gray-100 dark:bg-gray-800"></div>
    ...
    if (!ticket) {
        return (
            <div className="max-w-5xl mx-auto px-4 py-20 flex flex-col items-center justify-center text-center min-h-[85vh]">
                {/* Icon Wrapper */}
                ...
                {ticket.responses?.map((msg) => {
                    ...
                    >
                        ...
                        <div className={`flex flex-col ${isAdmin ? "items-end" : "items-start"} max-w-[75%]`}>
                            ...
                            {/* Bubble container */}
                            <div
                                className={`min-w-45 rounded-xl px-4 py-3 shadow-sm text-sm
        ${isAdmin
            ...
            {ticket.status !== "Resolved" && (
                <div className="flex justify-start items-center mt-4">
                    <Button variant="success-outline" onClick={() => handleUpdateTicket()} disabled={resolving}>
                        Mark as Resolved
                    </Button>
                    {resolving && <span className="ml-2 text-sm text-gray-500 dark:text-gray-400">Updating...</span>}
                </div>
            )}
            {/* Reply */}
            {ticket.status !== "Resolved" && (
