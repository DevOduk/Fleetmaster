...
const VERIFICATION_STEPS = [
  ...
  {
    key: "kra_pin",
    message: "Please upload your National ID document.",
    link: "/account/verify-id",
  },
  {
    key: "national_id",
    message: "Please submit your KRA PIN tax documentation.",
    link: "/account/verify-kra",
  },
  {
    key: "driving_license",
    message: "Please provide a valid Driver's License image.",
    link: "/account/verify-dl",
  },
] as const;
...
export default function VerificationBanner({ profile }: { profile?: any }) {
  ...
  const activeWarningStep = useMemo(() => {
    ...
    const verificationState = profile.verification_status as Record<
      string,
      boolean
    >;
    ...
    return (
      VERIFICATION_STEPS.find((step) => !verificationState[step.key]) || null
    );
  }, [profile]);
  ...
  return (
    ...
    >
      ...
      <div className="pr-6">
        ...
        {activeWarningStep.message}{" "}
        <Link
          href={activeWarningStep.link}
      ...
      <span
        ...
        className="absolute top-1/2 right-2 -translate-y-1/2 cursor-pointer text-amber-600 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-200"
      >
