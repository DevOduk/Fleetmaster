...
export default function ViewBooking({ BookingID }: { BookingID: number; }) {
  ...
  const grossSubTotal = useMemo(() => {
    return bookingDetails?.vehicle?.daily_rate * (newBookingDetails?.rental_days - bookingDetails?.rental_days) || 0;
  }, [bookingDetails, newBookingDetails]);

  const vatAmount = useMemo(() => {
    return Math.round((bookingDetails?.vehicle?.daily_rate * (newBookingDetails?.rental_days - bookingDetails?.rental_days)) * 0.16);
  }, [bookingDetails, newBookingDetails]);
  ...
  const handleCheckoutSubmit = async () => {
    ...
    setIsPaying(true);
    const firstName = bookingDetails?.renter_name.split(' ')[0]; // Keeping your custom profile schema spelling
    const lastName = bookingDetails?.renter_id.split(' ')[1];
    ...
    if (paymentMethod === 'm-pesa') {
      ...
      try {
        ...
        const res = await fetch('/api/intasend/stk', {
          ...
          body: JSON.stringify({
            amount: Number(bookingDetails.total + 0),
            phone: sanitizedNumber,
            email: null,
            firstName,
            lastName,
            vehicleID: Number(bookingDetails.vehicle?.id),
            rentalStart: bookingDetails.rental_start,       // 'YYYY-MM-DD'
            rentalEnd: bookingDetails.rental_start,           // 'YYYY-MM-DD'
            rentalTime: bookingDetails.rental_time,      // 'HH:mm:ss'
            rentalDays: Number(bookingDetails.rental_days),
            tenantID: profile?.tenant_id,
            userID: null,
            nationalID: bookingDetails?.renter_id,
  ...
  // console.log(newBookingDetails === bookingDetails)
    ...
    <div className="p-5 border-gray-200 rounded-2xl dark:border-gray-800">
      {bookingDetails ?
        <div className="grid grid-cols-12 gap-6">
          <div className="flex flex-col px-2 relative col-span-12 lg:col-span-7">
            ...
            <div>
              ...
              <Button className="w-full mt-6" variant="danger" size="sm">{'Cancel Booking'}</Button>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 shadow-sm">
              ...
              <div className="space-y-3 mb-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const startDay = dayjs(bookingDetails.rental_start);
                        const endDay = dayjs(newDate);

                        const dayGap = startDay.isValid() && endDay.isValid() ? endDay.diff(bookingDetails.rental_start, "day") : 0;
                        ...
                        setNewBookingDetails((prev) => ({
                          ...
                          rental_days: dayGap <= 0 ? bookingDetails.rental_days : dayGap,
                        }));
                      }}
                      name="end_date"
                    />
                    ...
                    onClick={() => {
                      setNewBookingDetails((prev) => ({
                        ...
                        rental_end: new Date(new Date(bookingDetails.rental_end).getTime() + day * 24 * 60 * 60 * 1000)
                          .toISOString()
                          .split('T')[0],
                        rental_days: bookingDetails.rental_days + day
                      }));
                    }}>
                    <PlusIcon />
                    {day} Day{day > 1 ? 's' : ''}
                ...
                <div className="flex items-center justify-between text-xs text-gray-500 bg-gray-50 dark:bg-gray-950 p-2 rounded border dark:border-gray-800">
                  ...
                  <span className="font-bold text-gray-800 dark:text-gray-200">{newBookingDetails?.rental_days - bookingDetails?.rental_days} Days</span>
                </div>
              </div>
              ...
              {expandBreakdown && (
                <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-950/40 text-sm space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-gray-800 dark:text-gray-200">{newBookingDetails?.rental_days - bookingDetails?.rental_days} Days</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 dark:text-gray-400">Base Subtotal ({newBookingDetails?.rental_days - bookingDetails?.rental_days}d × Ksh {grossSubTotal.toLocaleString()})</span>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative mt-2">
                      <Input
                        ...
                        className="pl-15.5"
                        defaultValue={bookingDetails?.renter_phone}
                        value={mpesaNumber || bookingDetails?.renter_phone}
