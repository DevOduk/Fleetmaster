...
function StatisticsBanner({ tenant }: { tenant: any; }) {
    ...
    const yardCount = tenant?.yards && Array.isArray(tenant.yards) ? tenant.yards.length : 0;
    ...
    return (
        <div className="mb-5 text-gray-900 container mx-auto rounded-3xl p-6 bg-gray-100 dark:bg-gray-900 shadow shadow-brand-400">
            {/* Header Section */}
            <div className="flex justify-between items-center mb-8">
                ...
                <div className="hidden sm:block">
                    <div className="flex items-center gap-4">
                        <Link id="heroactionBtn" className="text-sm ml-2 p-2 rounded-lg px-4 font-medium bg-blue-500 flex items-center text-white transition-colors" href="/vehicles">
                    ...
                    <div key={index} className="mb-4 md:mb-0">
                        <div className="stat-number text-2xl font-bold text-brand-500">
                            {item.isLoading ? (
                                ...
                                <CountUp end={item.end} duration={5} decimals={item.decimals || 0} />
                            )}
                            {item.unit || "+"}
                        </div>
