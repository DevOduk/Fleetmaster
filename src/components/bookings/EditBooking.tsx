...
export function getTimeRemaining(
  ...
  if (status.toLowerCase() === "reserved") {
    if (
      new Date().getTime() >
      new Date(created_at).getTime() + 30 * 60 * 1000
    ) {
...
export default function ViewBooking({ BookingID }: { BookingID: number }) {
  ...
  const updateBookingStatus = async (status: string) => {
    ...
    if (response.success) {
      showToast(
        `Booking #${BookingID} status updated successfully!`,
        "success",
      );
  ...
  const handleCheckoutSubmit = async () => {
    ...
    if (
      !newBookingDetails.rental_end ||
      new Date(
        `${newBookingDetails.rental_end}T${newBookingDetails.rental_time}`,
      ) < new Date(`${bookingDetails.rental_end}T${bookingDetails.rental_time}`)
    ) {
  ...
  const handleCancellation = async () => {
    ...
    if (response.success) {
      showToast(
        `Booking #${BookingID} has been cancelled successfully! Check your email for further instructions.`,
        "success",
      );
      setBookingDetails((prev) => ({
        ...prev,
  ...
  return (
    <div className="rounded-2xl border-gray-200 p-5 dark:border-gray-800">
      {/* cancel booking */}
      <Modal
        isOpen={isOpen}
        onClose={closeModal}
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
            ...
            <div className="col-span-1 sm:col-span-2">
              ...
              <Select
                ...
                options={
                  profile?.role === "Client" ? clientReasons : hostReasons
                }
              />
            </div>
            ...
            <div className="col-span-1 sm:col-span-2">
              ...
              <span className="mt-1 block text-xs text-gray-500">
                Please enter the account email to verify and trigger status
                update.
              </span>
            </div>
          </div>
      ...
      {bookingDetails ? (
        <div className="grid grid-cols-12 gap-6">
          <div className="relative col-span-12 flex flex-col px-2 xl:col-span-7">
            <Button
            ...
            <div>
              ...
              {isCancelled ? (
                <div className="mt-6 w-full p-3 text-center">
                  <p className="text-sm font-medium text-red-600 dark:text-red-500">
                    This booking has been cancelled with reason "
                    {bookingDetails?.cancellation_reason || "Unknown"}"!
                  </p>
                </div>
              ) : isClient ? (
                <Button
              ...
              {!isCancelled && (
                <p className="py-4 text-center text-sm text-gray-500">
                  Cancellations charges may apply.{" "}
                </p>
              )}
            </div>
          </div>
            ...
            {!isReviewed ? ((isCompleted || isCancelled) ? (
              ...
              <>
                <Alert
                  title="Booking Ended!"
                  variant="info"
                  message="This booking has ended! This booking will be complete once post rental assessments and pending issues/claims are solved!"
                />
                <Link href={"/vehicles"}>
                  {profile.role === "Client" && (
                    <Button className="w-full" size="md" variant="success">
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <div className="mb-4 space-y-3">
                  ...
                  <small className="mb-2 text-sm text-red-500">
                    {new Date(
                      `${newBookingDetails.rental_end}T${newBookingDetails.rental_time}`,
                    ) <
                      new Date(
                        `${bookingDetails.rental_end}T${bookingDetails.rental_time}`,
                  ...
                  <div className="flex items-center justify-between rounded border bg-gray-50 p-2 text-xs text-gray-500 dark:border-gray-800 dark:bg-gray-950">
                    <span className="flex items-center gap-1">
                      <ScheduleIcon fontSize="inherit" /> Computed Extension
                      Span:
                    </span>
                ...
                {expandBreakdown && (
                  <div className="space-y-3 rounded-xl border border-gray-100 bg-gray-50/50 p-4 text-sm dark:border-gray-800 dark:bg-gray-950/40">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-gray-800 dark:text-gray-200">
                        Ksh. {grossSubTotal.toLocaleString() || 0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-gray-500 dark:text-gray-400">
                ...
                <Modal
                  ...
                  className="z-99999 max-w-150 bg-white! p-5 lg:p-10 dark:bg-black!"
                >
