...
function SearchModal({ isOpen, setIsOpen, Tickets, Bookings, Vehicles, PAGES }: any) {
        ...
        const results = [
            {
                ...
                items: Vehicles
                    .filter(v =>
                        `${v.make} ${v.model} ${v.year} ${v.category} ${v.description}`.toLowerCase().includes(lowerQuery)
                    )
                    .map(v => ({ title: `${v.make} ${v.model}`, desc: `Ksh. ${v.daily_rate.toLocaleString()} | ${v.status} | ${v.description}`, link: `/vehicles/${v.id}` }))
            },
            {
                category: "Bookings",
                items: bookings
                    .filter(b => (`${b.id} ${b.renter_name} ${b.booking_status} ${b.vehicleDetails.make} ${b.vehicleDetails.model}`)?.toLowerCase().includes(lowerQuery))
                    .map(b => ({ title: `Booking #${b.id}`, desc: `View rental booking | Date created: ${new Date(b.created_at).toLocaleString()}`, link: `/bookings/${b.id}` }))
            },
            {
                category: "Support Tickets",
                items: tickets
                    .filter(t => `${t.ticket_number} ${t.subject} ${t.description}`?.toLowerCase().includes(lowerQuery))
                    .map(t => ({ title: t.ticket_number, desc: `${t.status} | ${t.category} | ${t.description}`, link: `/support/tickets/${t.ticket_number.replace('#', '')}` }))
            },
            {
                category: "Help & Info",
                items: PAGES
                    .filter(p => `${p.title} ${p.description}`?.toLowerCase().includes(lowerQuery))
        ...
        >
            ...
            <div className="max-h-125 overflow-auto custom-scrollbar flex flex-col gap-3">
                {searchResults.length > 0 ? searchResults?.map((group) => (
                        ...
                        <div className="flex flex-col gap-3 p-2">
                            {group.items.map((item, idx) => (
                                <Link
                                    ...
                                    target="_blank"
                                    href={item.link}
                                    className="group flex gap-3 items-center text-gray-500 border border-gray-300 dark:border-gray-800 p-3 rounded-xl cursor-pointer hover:border-brand-500 hover:text-brand-500 hover:bg-brand-500/10 hover:dark:border-brand-500 hover:dark:text-brand-500 transition-all duration-200"
                                >
                                    {/* Dynamic Icon Helper */}
                                    {getIcon(group.category)}

                                    <div className="w-full relative overflow-hidden">
                                        <p className="text-black dark:text-white text-md group-hover:text-brand-500 dark:group-hover:text-brand-500">
                                            {item.title}
                                        </p>
                                        <p className="text-gray-500 w-full truncate dark:text-gray-600 text-sm group-hover:text-brand-500/80 dark:group-hover:text-brand-500/80">
                                            {item.desc}
                                        </p>
                                    </div>
                    ...
                    <div className="w-full flex items-center h-40 justify-center text-gray-500">
                        <p>
